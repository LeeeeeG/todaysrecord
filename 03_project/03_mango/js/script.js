$(function(){
    ...
    //배너슬라이드
    const slideBanner=$('.banner-wrap .area'); //ul이 통째로 움직임
    const slideList=$('.banner-wrap .area li');
    const slideWidth=slideList.width();
    const slideOn=$('.banner-wrap .area li .on');
    let current1=0;
    let current2=3;
    let set=null;
    /* console.log(slideWidth) 로 넓이 확인 */

    bannerSlider();
    function bannerSlider(){
    set=setInterval(/* function({})과 () => {}은 똑같음*/() => {
        /* var i=$('.banner-wrap .area li').index();
        console.log(i); */
        slideBanner.stop().animate({left:-slideWidth},500,
        function(){
